...
const Page = () => {
    const auth = useAuth()
    const {data, isPending, error} = useMyAds()

    if (error?.response?.data?.detail === "This account is banned.") {
        return <div className="h-screen flex items-center justify-center">
                <p dir="rtl" className="bg-red-600 py-2 text-white px-3 rounded-lg font-[Yekan-Bold]">حساب کاربری شما مسدود شده است.</p>
            </div>
    }
