    ...
    return (
        <div className="pt-44 w-full min-[475px]:w-4/5 min-[640px]:w-3/5 min-[900px]:w-2/5 px-2 m-auto h-screen">
            <form className="bg-gradient-to-b from-white to-gray-200 w-full py-16 px-6 rounded-lg shadow-lg flex flex-col gap-6" onSubmit={formSubmitHandler}>
                <h1 className="font-[Yekan-Bold] text-lg mx-auto">فرم تغییر رمز عبور</h1>
                <div className="relative flex flex-col gap-2 items-end">
                    <label className="font-[Yekan-Medium]">رمز عبور قبلی</label>
                    {oldPasswordValue.length > 0 && showOldPassword === "show" ? <AiFillEye size={20} className="text-gray-500 absolute bottom-3 right-2 cursor-pointer" onClick={showOldPasswordHandler} /> : oldPasswordValue.length > 0 && showOldPassword === "hidden" ? <AiFillEyeInvisible size={20} className="text-gray-500 absolute bottom-3 right-2 cursor-pointer" onClick={showOldPasswordHandler} />: undefined}
                    <input type={showOldPassword === "show" ? "password" : "text"} name="oldPassword" className="border-2 rounded-md outline-1 py-2 px-3 w-full" onChange={(event) => setOldPasswordValue(event.target.value)} />
                </div>
                <div className="relative flex flex-col gap-2 items-end">
                    <label className="font-[Yekan-Medium]">رمز عبور جدید</label>
                    {newPasswordValue.length > 0 && showNewPassword === "show" ? <AiFillEye size={20} className="text-gray-500 absolute bottom-3 right-2 cursor-pointer" onClick={showNewPasswordHandler} /> : newPasswordValue.length > 0 && showNewPassword === "hidden" ? <AiFillEyeInvisible size={20} className="text-gray-500 absolute bottom-3 right-2 cursor-pointer" onClick={showNewPasswordHandler} />: undefined}
                    <input type={showNewPassword === "show" ? "password" : "text"} placeholder="mM@123456" name="newPassword" className="border-2 rounded-md outline-1 py-2 px-3 w-full" onChange={(event) => setNewPasswordValue(event.target.value)} />
                </div>
                <div className="relative flex flex-col gap-2 items-end">
                    <label className="font-[Yekan-Medium]">تکرار رمز عبور جدید</label>
                    {confirmPasswordValue.length > 0 && showConfirmPassword === "show" ? <AiFillEye size={20} className="text-gray-500 absolute bottom-3 right-2 cursor-pointer" onClick={showConfirmPasswordHandler} /> : confirmPasswordValue.length > 0 && showConfirmPassword === "hidden" ? <AiFillEyeInvisible size={20} className="text-gray-500 absolute bottom-3 right-2 cursor-pointer" onClick={showConfirmPasswordHandler} />: undefined}
                ...
                <div className="flex flex-col mx-auto gap-3">
                    <button className="hover:text-white hover:bg-gradient-to-tr from-orange-700 to-orange-300 px-4 transition rounded-md py-2 font-[Yekan-Medium] disabled:pointer-events-none disabled:text-gray-600" disabled={isSubmitDisabled || isLoading} type="submit">{isLoading ? "صبر کنید" : "تغییر رمز عبور"}</button>
                </div>
