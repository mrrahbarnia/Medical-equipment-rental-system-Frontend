...
import { errorHandler, contextMessageHandler } from "@/app/utils/messageUtils";
import Link from "next/link";
import { useRouter } from "next/navigation";

const INTERNAL_LOGIN_API: string = "/api/login/"
...
const Page = () => {
    ...
    const isSubmitDisabled = emailValue.length === 0 || passwordValue.length === 0
    return (
        <Fragment>
            ...
            <div className="pt-44 w-full min-[475px]:w-4/5 min-[640px]:w-3/5 min-[900px]:w-2/5 px-2 m-auto h-screen">
                <form className="w-full py-16 px-6 rounded-lg shadow-sm shadow-blue-900 flex flex-col gap-6" onSubmit={formSubmitHandler}>
                    ...
                    <div className="flex flex-col gap-2 items-end">
                        ...
                        <input type="email" placeholder="example@email.com" name="username" className="rounded-md outline-1 py-2 px-3 w-full" onChange={(event) => setEmailValue(event.target.value)} value={emailValue} />
                    </div>
                    ...
                    <div className="flex flex-col mx-auto gap-3">
                        ...
                        <Link href="/accounts/reset-password" className="text-white font-[Yekan-Medium]">رمز عبور خود را فراموش کرده ام</Link>
                    </div>
