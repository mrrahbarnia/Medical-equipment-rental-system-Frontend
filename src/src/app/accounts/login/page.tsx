        ...
        <Fragment>
            ...
            <div className="pt-44 w-full min-[475px]:w-4/5 min-[640px]:w-3/5 min-[900px]:w-2/5 px-2 m-auto h-screen">
                <form className="bg-gradient-to-b from-white to-gray-200 w-full py-16 px-6 rounded-lg shadow-lg flex flex-col gap-6" onSubmit={formSubmitHandler}>
                    <h1 className="font-[Yekan-Bold] text-lg mx-auto">فرم ورود</h1>
                    <div className="flex flex-col gap-2 items-end">
                        <label className="font-[Yekan-Medium]">شماره موبایل</label>
                        <input type="text" placeholder="09131234567" name="username" className="rounded-md outline-1 border-2 py-2 px-3 w-full" onChange={(event) => setPhoneNumberValue(event.target.value)} value={phoneNumberValue} />
                    </div>
                    <div className="relative flex flex-col gap-2 items-end">
                        ...
                        <input type={showPassword === "show" ? "password" : "text"} name="password" className="rounded-md outline-1 border-2 py-2 px-3 w-full" onChange={(event) => setPasswordValue(event.target.value)} value={passwordValue} />
                    </div>
                    <div className="flex flex-col mx-auto gap-3">
                        <button className="hover:text-white hover:bg-gradient-to-tr from-orange-700 to-orange-300 w-24 transition mx-auto rounded-md py-2 font-[Yekan-Medium] disabled:pointer-events-none disabled:text-gray-400" disabled={isSubmitDisabled || isLoading} type="submit">{isLoading ? "صبر کنید" : "ورود"}</button>
                        <Link href="/accounts/reset-password" className="font-[Yekan-Medium] hover:text-gray-600">رمز عبور خود را فراموش کرده ام</Link>
                    </div>
